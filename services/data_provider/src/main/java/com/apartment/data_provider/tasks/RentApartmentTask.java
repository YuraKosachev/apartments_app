...
public class RentApartmentTask extends BaseApartmentTask {
    ...
    public void run() {
        ...
        try {
            ...
            log.info(String.format("RentApartment task finished inserted: %d deleted: %d, updated: %d source:%d. Parsing took -> %s", inserted.size(), deleted.size(), updated.size(), source.size(), parsingTime));
        }catch (Exception e){
